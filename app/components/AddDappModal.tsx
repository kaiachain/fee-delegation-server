  ...
  const handleBalanceThresholdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^\d*\.?\d*$/.test(value)) {
      ...
      if (cleanValue === '' || cleanValue === '.') {
        ...
        // Don't convert yet if ending with decimal point
        setNewEmailAlert({...newEmailAlert, balanceThreshold: 0}); // Set to 0 temporarily to allow typing
        return;
      } else {
        numValue = Number(cleanValue) || 0;
      }
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="text-lg font-bold text-gray-800">Email Alerts</h3>
                </div>
              </div>

              <div className="space-y-4 bg-green-50 border-l-4 border-green-400 rounded-lg p-4 shadow-sm">
                <div className="flex items-center space-x-2">
                  <svg className="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <h3 className="text-lg font-medium text-green-800">Balance Alerts</h3>
                </div>
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-green-700">
                        Set up email alerts to be notified when your DApp balance falls below specified thresholds.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        type="email"
                        value={newEmailAlert.email}
                        onChange={(e) => setNewEmailAlert({...newEmailAlert, email: e.target.value})}
                        className="w-full px-4 py-2.5 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-900 placeholder-gray-500 transition-colors duration-200"
                        placeholder="Enter email address"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-sm font-medium text-gray-700">Balance Threshold</label>
                      <div className="relative">
                        <input
                          type="number"
                          step="0.000000000000000001"
                          ...
                          value={(() => {
                            // Display cleaned value without leading zeros, show 0 when cleared
                            if (newEmailAlert.balanceThreshold === 0) return '0';
                            const thresholdStr = newEmailAlert.balanceThreshold.toString();
                            if (thresholdStr.includes('.')) {
                              const [wholePart, decimalPart] = thresholdStr.split('.');
                              const cleanWholePart = wholePart.replace(/^0+/, '') || '0';
                              return cleanWholePart + '.' + decimalPart;
                            } else {
                              return thresholdStr.replace(/^0+/, '') || '0';
                            }
                          })()}
                          onChange={handleBalanceThresholdChange}
                          className="w-full pl-4 pr-12 py-2.5 border border-green-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-900 placeholder-gray-500 transition-colors duration-200"
                    ...
                    <div className="flex items-end space-x-2 h-full">
                      <div className="flex items-center space-x-2 py-2.5">
                        <input
                          type="checkbox"
                          id="alertActive"
                          checked={newEmailAlert.isActive}
                          onChange={(e) => setNewEmailAlert({...newEmailAlert, isActive: e.target.checked})}
                          className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded transition-colors duration-200"
                        />
                        <label htmlFor="alertActive" className="text-sm font-medium text-gray-700">
                          Active
                        </label>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-end">
                    <button
                      onClick={() => {
                        if (newEmailAlert.email && newEmailAlert.balanceThreshold > 0) {
                          setEmailAlerts([...emailAlerts, {...newEmailAlert}]);
                          setNewEmailAlert({email: "", balanceThreshold: 0, isActive: true});
                        } else {
                          setErrorModal({
                            isOpen: true,
                            title: "Invalid Alert",
                            message: "Please provide a valid email address and balance threshold."
                          });
                        }
                      }}
                      className="px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 shadow-sm hover:shadow"
                    >
                      Add Alert
                    </button>
                  </div>
                        ...
                        >
                          <div className="flex-1">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm text-green-700">{alert.balanceThreshold} KAIA</span>
                              <span className="text-xs text-gray-500">•</span>
                              <span className={`text-xs px-2 py-1 rounded-full ${alert.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
                                {alert.isActive ? 'Active' : 'Inactive'}
                              </span>
                            </div>
                          </div>
                          <button
                            onClick={() => setEmailAlerts(emailAlerts.filter((_, i) => i !== index))}
