...
import ContractUsageTable from "./ContractUsageTable";
...
const DappCard: React.FC<DappCardProps> = ({ dapp, children, deleteDapp }) => {
  ...
  const [dappInfo, setDappInfo] = useState<Dapp>({
    ...dapp,
    contractUsages: dapp.contractUsages || [],
  ...
  const handleDappUpdate = (updatedDapp: Dapp) => {
    setDappInfo((prev) => ({
      ...prev,
      ...updatedDapp,
      contractUsages: Array.isArray(updatedDapp.contractUsages) && updatedDapp.contractUsages.length > 0
        ? updatedDapp.contractUsages
        : prev.contractUsages,
    }));
  };
  const usageFor = (addr: string) => {
    const usage = (dappInfo.contractUsages || []).find(
      (entry) => entry.contractAddress?.toLowerCase() === addr.toLowerCase()
    );
    return usage?.totalUsed || null;
  };

  const renderUsageBadge = (addr: string) => {
    const totalUsed = usageFor(addr);
    if (!totalUsed) {
      return (
        <div className="px-2 py-0.5 text-xs font-medium bg-white border border-dashed border-blue-200 rounded text-blue-400 whitespace-nowrap">
          No usage yet
        </div>
      );
    }
    return (
      <div className="px-2 py-0.5 text-xs font-medium bg-white border border-blue-200 rounded text-blue-700 whitespace-nowrap">
        Used {formatBalance(totalUsed)} KAIA
      </div>
    );
  };
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-4" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Active/Inactive Toggle Button - Only for Super Admin */}
            {session?.user?.role === 'super_admin' ? (
              ...
              /* Status Display Only for Non-Super Admin */
              <div className={`inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg ${
                dappInfo.active 
                  ? 'text-green-700 bg-green-100 border border-green-200' 
                  : 'text-red-700 bg-red-100 border border-red-200'
              }`}>
                <svg className={`w-4 h-4 mr-2 ${dappInfo.active ? 'text-green-700' : 'text-red-700'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  {dappInfo.active ? (
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  ) : (
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
            ...
            {deleteDapp && dappInfo.id && session?.user?.role === 'super_admin' && (
              <DelDappBtn
                dapp={dappInfo}
        ...
        <div className="space-y-4">
          <ContractUsageTable
            usages={dappInfo.contractUsages || []}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Assigned Users Section - Only visible to SUPER_ADMIN */}
            {session?.user?.role === 'super_admin' && (dappInfo as any).assignedUsers && (
              <div className="mt-6">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2 bg-gradient-to-br from-indigo-100 via-indigo-50 to-purple-50 px-3 py-1.5 rounded-lg border border-indigo-200 shadow-sm">
                    <svg className="w-4 h-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    <h3 className="text-base font-semibold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Assigned Users</h3>
                    <span className="px-2 py-0.5 text-xs font-medium bg-indigo-100 text-indigo-800 rounded-full">
                      {((dappInfo as any).assignedUsers || []).length}
                    </span>
                  </div>
                </div>
                <div className="space-y-2">
                  {((dappInfo as any).assignedUsers || []).length > 0 ? (
                    ((dappInfo as any).assignedUsers || []).map((user: any) => (
                      <div
                        key={user.id}
