...
async function main() {
  ...
  if (![routerAddress, usdtAddress, wkaiaAddress].every(ethers.isAddress)) {
    throw new Error("UNISWAP_ROUTER, USDT_ADDRESS, and WKAIA_ADDRESS must be valid addresses");
  }

  const Swap = await ethers.getContractFactory("GaslessERC20PermitSwap", wallet);
  const swap = await Swap.deploy(routerAddress, usdtAddress, wkaiaAddress);
